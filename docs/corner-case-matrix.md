...
| `eco-external` | `shapes eco-external` | checkout SERVER root + CLIENT child carrying `server.address=api.stripe.test`, with no matching instrumented SERVER child | Ecosystem graph (plan 166) | One external HTTP node named `api.stripe.test`; checkout → external edge; no duplicate service node | |
...
| `a-breach-error-rate` | `a-breach-error-rate.sh` | flagd `paymentFailure` forced on: every checkout request fails for `BREACH_SECONDS` (default 200s, one request per 2s), flag restored on exit | Alerting v1 error-rate rule (plan 167) | Sustained ~100% checkout error rate breaches any >20% rule after its consecutive-breach hysteresis; incident opens and the triggered webhook fires | |
| `a-breach-p95` | `a-breach-p95.sh` | recommendation served with deterministic `?slow=` latency (default 900 ms) for `BREACH_SECONDS` (default 200s) | Alerting v1 p95-latency rule (plan 167) | Recommendation p95 exceeds a sub-900 ms threshold for the whole window; slow-p95 incident opens | |
| `a-recover` | `a-recover.sh` | `paymentFailure` forced off + sustained healthy checkout/recommendation traffic for `RECOVER_SECONDS` (default 200s) | Alerting v1 incident resolution (plan 167) | Open error-rate/latency incidents see consecutive healthy windows, resolve, and the resolved webhook fires | |
