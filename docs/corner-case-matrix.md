...
| `m-shapes` | `shapes m-shapes` | monotonic counter with mid-window reset, gauge with a 2-step gap, explicit histogram with exemplar | Metrics/dashboards | Rate handles the reset (no negative spike); gauge gap visible; exemplar deep-links to its trace | |
