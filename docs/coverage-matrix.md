...
| Malformed propagation | checkout best-effort ingress; notifications strict durable ingress | `propagation:malformed` | 10-case corrupt-carrier corpus stays paid on checkout; 7-case corpus is rejected 400 `invalid W3C propagation` by notify with zero rows persisted; valid control reaches Parallax as a queryable trace. |
| Cardinality stress | OTLP metrics → Parallax metrics API | `metrics:cardinality_stress` | Exactly 200 deterministic `stress_bucket` series stay enumerable through `metricLabelValues`; exact-set assertion, no unbounded churn. |
| Competitor fan-out | OTLP collector → Parallax + Jaeger + OpenObserve | `deploy/comparison/` | Digest-pinned latest-stable backends receive the identical workload; same-trace arrival proven in Parallax, Jaeger, and OpenObserve (`mise run check:fanout`, 14/14 spans); same-workload UI comparison pending. |
...
| Corpus dispatch | Rust scenario registry and Mise catalog | `check:scenarios`; `corpus:all` | 92 proofs exactly once: 63 A/B/C plus 29 corner proofs; unique public task names and no legacy wrapper scripts. This is dispatch evidence, not proof that every runtime journey passed. |
